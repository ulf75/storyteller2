...
class Helper {
    ...
    static GetRotatedPoint(transformedPoint, startPoint, angle) {
        return {
            x: transformedPoint.x * Math.cos(angle) +
                transformedPoint.y * Math.sin(angle) +
                startPoint.x,
            y: transformedPoint.y * Math.cos(angle) -
    ...
    static LimitPointToCircle(startPoint, radius, limitedPoint) {
        ...
        let x = Math.sqrt((Math.pow(radius, 2) * Math.pow(a - n, 2)) /
            (Math.pow(a - n, 2) + Math.pow(b - m, 2))) + a;
        if (limitedPoint.x < 0) {
            x *= -1;
        }
        let y = ((x - a) * (b - m)) / (a - n) + b;
...
class FlipCalculation {
    ...
    getCurrentClipArea() {
        const result = [];
        const startPoint = this.topIntersectPoint !== null ? this.topIntersectPoint : { x: this.pageWidth, y: 0 };
