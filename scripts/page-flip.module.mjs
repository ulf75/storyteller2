...
class Helper {
  ...
  static GetIntersectBetweenTwoSegment(rectBorder, one, two) {
    return Helper.PointInRect(
      rectBorder,
      Helper.GetIntersectBeetwenTwoLine(one, two)
    );
  }
  /**
...
class HTMLPage extends Page {
  ...
  drawSoft(position, commonStyle = "") {
    ...
    for (const p of this.state.area) {
      if (p !== null) {
        let g =
          this.render.getDirection() === 1 /* FlipDirection.BACK */
            ? {
  ...
  simpleDraw(orient, doClip = false) {
  ...
  setOrientation(orientation) {
    ...
    this.element.classList.add(
      orientation === 1 /* PageOrientation.RIGHT */ ? "--right" : "--left"
    );
  }
  setDrawingDensity(density) {
...
class PageCollection {
  ...
  getFlippingPage(direction) {
    const current = this.currentSpreadIndex;
    if (
      this.render.getOrientation() === "portrait" /* Orientation.PORTRAIT */
    ) {
      return direction === 0 /* FlipDirection.FORWARD */
      ...
      const spread =
        direction === 0 /* FlipDirection.FORWARD */
  ...
  showSpread() {
    ...
    if (spread.length === 2) {
      ...
      if (
        this.render.getOrientation() === "landscape" /* Orientation.LANDSCAPE */
      ) {
    ...
    for (const pageElement of this.pagesElement) {
      const page = new HTMLPage(
        ...
        pageElement.dataset["density"] === "hard"
          ? "hard" /* PageDensity.HARD */
          : "soft" /* PageDensity.SOFT */
      );
      page.load();
...
class FlipCalculation {
  ...
  calculateAngle(pos) {
    ...
    const top =
      this.corner === "bottom" /* FlipCorner.BOTTOM */
    ...
    if (this.corner === "bottom" /* FlipCorner.BOTTOM */) angle = -angle;
    return angle;
  }
...
class Flip {
  ...
  fold(globalPos) {
     ...
     */
    if (didStart) this.setState("user_fold" /* FlippingState.USER_FOLD */);
  ...
  flip(globalPos) {
    ...
    const yStart =
      this.calc.getCorner() === "bottom" /* FlipCorner.BOTTOM */
        ...
        if (direction === 1 /* FlipDirection.BACK */) {
          ...
          if (nextPage !== null) {
            if (this.flippingPage.getDensity() !== nextPage.getDensity()) {
              this.flippingPage.setDrawingDensity(
                "hard" /* PageDensity.HARD */
              );
  ...
  flipNext(corner) {
    this.flip({
      ...
      y:
        corner === "top" /* FlipCorner.TOP */
          ? 1
          : this.render.getRect().height - 2,
    });
  }
  /**
  ...
  showCorner(globalPos) {
    if (
      !this.checkState(
        "read" /* FlippingState.READ */,
        "fold_corner" /* FlippingState.FOLD_CORNER */
      )
    )
      return;
    const rect = this.getBoundsRect();
...
class UI {
  ...
  checkTarget(target) {
    return (
      ...
      Array.from(target.classList).some((value) =>
...
class Render {
   ...
   */
  setRightPage(page) {
    if (page !== null) page.setOrientation(1 /* PageOrientation.RIGHT */);
    this.rightPage = page;
  }
  /**
   ...
   */
  setLeftPage(page) {
    if (page !== null) page.setOrientation(0 /* PageOrientation.LEFT */);
    this.leftPage = page;
  }
  /**
...
class HTMLRender extends Render {
  ...
  drawHardInnerShadow() {
    ...
    let newStyle = `
        ...
        `;
    newStyle +=
      (this.getDirection() === 0 /* FlipDirection.FORWARD */ &&
        this.shadow.progress > 100) ||
  ...
  drawFrame() {
    ...
    if (this.shadow != null && this.flippingPage !== null) {
      if (
        this.flippingPage.getDrawingDensity() === "soft" /* PageDensity.SOFT */
      ) {
...
class Settings {
  constructor() {
    this._default = {
      ...
      bookType: "double" /* BookType.DOUBLE */,
      width: 0,
      height: 0,
      minWidth: 0,
      maxWidth: 0,
  ...
  getSettings(userSetting) {
    ...
    if (
      result.size !== "stretch" /* SizeType.STRETCH */ &&
      result.size !== "fixed" /* SizeType.FIXED */
    )
      throw new Error(
